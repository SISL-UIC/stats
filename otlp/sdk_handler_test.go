...
func TestDefaultResource(t *testing.T) {
	res, err := defaultResource(context.Background())
	if err != nil {
		t.Fatalf("defaultResource failed: %v", err)
	}

	attrs := make(map[string]struct{})
	for _, kv := range res.Attributes() {
		attrs[string(kv.Key)] = struct{}{}
	}

	// This test verifies the merge wires both sources together. Note that
	// resource.Default() is memoized on its first call process-wide, so the
	// values it derives from OTEL_SERVICE_NAME / OTEL_RESOURCE_ATTRIBUTES cannot
	// be reliably exercised here (another test may have frozen Default() before
	// this one set those vars). We assert on attribute *presence* instead:
	//
	//   - telemetry.sdk.* and service.name come from resource.Default()
	//   - host.name / process.pid come from the WithHost()/WithProcess()
	//     detectors merged on top
	for _, key := range []string{
		"telemetry.sdk.name", // from Default()
		"service.name",       // from Default() (real name or unknown_service fallback)
		"host.name",          // from the merged WithHost() detector
		"process.pid",        // from the merged WithProcess() detector
	} {
		if _, ok := attrs[key]; !ok {
			t.Errorf("merged resource missing expected attribute %q", key)
		}
	}
}
